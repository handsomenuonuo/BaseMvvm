...
class ProgressCellView : View {
    ...
    private fun init(context: Context, attrs: AttributeSet?) {
        ...
        if (attrs != null) {
            val ta = context.obtainStyledAttributes(attrs, R.styleable.my_ProgressCellView)
            primaryColor = ta.getColor(R.styleable.my_ProgressCellView_pc_primaryColor, primaryColor)
            accentColor = ta.getInteger(R.styleable.my_ProgressCellView_pc_accentColor, accentColor)
            totalCount = ta.getColor(R.styleable.my_ProgressCellView_pc_totalCount, totalCount)
            accentCount = ta.getInteger(R.styleable.my_ProgressCellView_pc_accentCount, accentCount)
            cellWidth =
                ta.getDimension(R.styleable.my_ProgressCellView_pc_cellWidth, cellWidth.toFloat())
                    .toInt()
            cellSpace =
                ta.getDimension(R.styleable.my_ProgressCellView_pc_cellSpace, cellSpace.toFloat())
                    .toInt()
            reverse = ta.getBoolean(R.styleable.my_ProgressCellView_pc_reverse, reverse)
            oritenation = ta.getInteger(R.styleable.my_ProgressCellView_pc_oritenation, oritenation)
            ta.recycle()
        }
    }
