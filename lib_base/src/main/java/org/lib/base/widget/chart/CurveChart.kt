package org.lib.base.widget.chart
...
class CurveChart : View {
    ...
    private fun initAttr(context: Context, attrs: AttributeSet?) {
        if (attrs != null) {
            val ta = context.obtainStyledAttributes(attrs, R.styleable.my_CurveChart)
            xAxisColor = ta.getColor(R.styleable.my_CurveChart_cc_xAxisColor, xAxisColor)
            xAxisWidth = ta.getDimension(R.styleable.my_CurveChart_cc_xAxisWidth, xAxisWidth)
            xAxisTextColor =
                ta.getColor(R.styleable.my_CurveChart_cc_xAxisTextColor, xAxisTextColor)
            xAxisTextSize =
                ta.getDimension(R.styleable.my_CurveChart_cc_xAxisTextSize, xAxisTextSize)
            xAxisTextRowLeftMargin = ta.getDimension(
                R.styleable.my_CurveChart_cc_xAxisTextRowLeftMargin,
                xAxisTextRowLeftMargin
            )
            xAxisTextRowRightMargin = ta.getDimension(
                R.styleable.my_CurveChart_cc_xAxisTextRowRightMargin,
                xAxisTextRowRightMargin
            )
            xAxisTextRowTopMargin = ta.getDimension(
                R.styleable.my_CurveChart_cc_xAxisTextRowTopMargin,
                xAxisTextRowTopMargin
            )
            yAxisColor = ta.getColor(R.styleable.my_CurveChart_cc_yAxisColor, yAxisColor)
            yAxisWidth = ta.getDimension(R.styleable.my_CurveChart_cc_yAxisWidth, yAxisWidth)
            yAxisTextColor =
                ta.getColor(R.styleable.my_CurveChart_cc_yAxisTextColor, yAxisTextColor)
            yAxisTextSize =
                ta.getDimension(R.styleable.my_CurveChart_cc_yAxisTextSize, yAxisTextSize)
            yAxisTextColRightMargin = ta.getDimension(
                R.styleable.my_CurveChart_cc_yAxisTextColRightMargin,
                yAxisTextColRightMargin
            )
            yAxisTextColTopMargin = ta.getDimension(
                R.styleable.my_CurveChart_cc_yAxisTextColTopMargin,
                yAxisTextColTopMargin
            )
            showYAxisLine = ta.getBoolean(R.styleable.my_CurveChart_cc_showYLine, showYAxisLine)
            xMarkLineColor =
                ta.getColor(R.styleable.my_CurveChart_cc_xMarkLineColor, xMarkLineColor)
            xMarkLineWidth =
                ta.getDimension(R.styleable.my_CurveChart_cc_xMarkLineWidth, xMarkLineWidth)
            yMarkLineColor =
                ta.getColor(R.styleable.my_CurveChart_cc_yMarkLineColor, yMarkLineColor)
            yMarkLineWidth =
                ta.getDimension(R.styleable.my_CurveChart_cc_yMarkLineWidth, yMarkLineWidth)
            showXMarkLine =
                ta.getBoolean(R.styleable.my_CurveChart_cc_showXMarkLine, showXMarkLine)
            showYMarkLine =
                ta.getBoolean(R.styleable.my_CurveChart_cc_showYMarkLine, showYMarkLine)
            curveColor = ta.getColor(R.styleable.my_CurveChart_cc_curveColor, curveColor)
            curveWidth = ta.getDimension(R.styleable.my_CurveChart_cc_curveWidth, curveWidth)
            ta.recycle()
        }
    }
